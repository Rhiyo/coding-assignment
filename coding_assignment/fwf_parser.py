...
class FwfParser():
    ...
    def generate_fwf(self, to_path : str, rows: int = 1) -> None:
        ...
        with open(to_path, 'w', encoding=self.spec['FixedWidthEncoding']) as f:
            if(self.spec["IncludeHeader"] == "True"):
                f.write((''.join([col_name.ljust(int(self.spec['Offsets'][i])) for i, col_name in enumerate(self.spec["ColumnNames"])]) + "\n"))
            ...
            for _ in range(rows):
                ...
                for offset in self.spec['Offsets']:
                    offset = int(offset)
                    random_chars = ''.join(random.choices(string.ascii_uppercase + string.digits, k=random.randint(0, offset)))
    ...
    def to_csv(self, fw_path : str, to_path : str) -> None:
        ...
        with open(fw_path, 'r', encoding=self.spec['FixedWidthEncoding']) as read_file:
            with open(to_path, 'w', encoding=self.spec['DelimitedEncoding']) as write_file:
                while read_file:
                    ...
                    for offset in self.spec['Offsets']:
                        ...
                        col_values.append(line[offset_sum:offset_sum + offset].strip())
