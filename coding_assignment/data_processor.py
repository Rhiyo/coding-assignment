...
def generate_user_csv(to_path: str, user_faker: UserFaker, size_mb: float = 50) -> None:
    ...
    with open(to_path, "wb") as f:
        header_row = (
            ",".join(["first_name", "last_name", "address", "date_of_birth"]) + "\n"
        ).encode("utf-8")
        ...
        while True:
            ...
            row = (
                ",".join([first_name, last_name, address, birth_date]) + "\n"
            ).encode("utf-8")
...
def anonymize_user_data_by_line(
    csv_path: str, to_path: str, user_faker: UserFaker
) -> None:
    """
...
def anonymize_user_data_multiprocess(
    csv_path: str, to_path: str, user_faker: UserFaker, n_jobs: int = -1, seed: int = -1
) -> None:
    """
    Anonymize fields first_name, last_name and address of user csv file
    with fake values.
    ...
    for chunkStart, chunkSize in chunks:
        jobs.append(
            pool.apply_async(
                process_chunk, (csv_path, user_faker, chunkStart, chunkSize, seed)
            )
        )
